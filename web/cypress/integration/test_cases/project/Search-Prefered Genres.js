...
describe('Sprint #2 Testing' , () => {
    ...
    //test Prefered Genres
    it('Your Library Module & Artist Components' , () => {
        //library logic
        cy.get('[href="/home/library/playlists"]').click();
        //Artist (No Artist yet to be tested)
        ...
        cy.get('#cardID').eq(1).click();
        cy.go('back');
        cy.contains('lala').click();
        cy.go('back');
        cy.get('#cardID').eq(2).click();
        cy.go('back');
        cy.contains('consequatur').click();
        cy.go('back');
    })
})